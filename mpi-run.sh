...
#    lines=$(sort $HOST_FILE_PATH|uniq|wc -l)
  ...
  timeout 1800 time mpirun --mca btl_tcp_if_include eth0 --allow-run-as-root -np ${AWS_BATCH_JOB_NUM_NODES} --hostfile combined_hostfile /hordesat/hordesat  -c=${NUM_PROCESSES} -d=7 supervised-scripts/test.cnf
}
